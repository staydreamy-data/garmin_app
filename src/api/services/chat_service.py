...
class ChatService:
    ...
    def handle_chat_turn(self, request: ChatRequest) -> ChatResponse:
        ...
        if request.session_id is None:
            chat_session = create_chat_session(self.db)
        else:
            chat_session = get_chat_session(self.db, request.session_id)
            if chat_session is None:
                raise SessionNotFoundError

        all_session_messages = list(get_messages_by_session(self.db, chat_session.id))
        ...
        if not chat_session.title:
            ...
            chat_session = update_chat_session_title(self.db, chat_session, title)
        ...
        try:
            maybe_compact_session_context(self.db, chat_session)
