...
splits_union as (
    select
        ...
        run_date,
        (
            select distinct workout_name from {{ ref('running_workouts') }}
            where workout_id = w.workout_id
        ) as workout_name,
        distance_m,
        duration_sec,
        case when step_type = 'warmup' then 0 else split_index end
            as split_index,
        {{ format_pace_from_speed('average_speed_ms') }} as split_pace,
        case when step_type = 'warmup' then 0 else workout_step_index end
            as workout_step_index,
        step_type,
