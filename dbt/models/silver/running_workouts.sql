...
top_level_steps as (
    select
        s.workout_id,
        s.workout_name,
        cast(ws.value ->> '$.stepOrder' as integer) as step_order, -- Subtract 1 from step order to align with splits
        ...
        cast(ws.value ->> '$.targetType.workoutTargetTypeKey' as string) as target_type,
...
(select 
        s.workout_id,
        s.workout_name,
        cast(ns.value ->> '$.stepOrder' as integer) as step_order, -- Subtract 1 from step order to align with splits
        ...
        cast(ns.value ->> '$.targetType.workoutTargetTypeKey' as string) as target_type,
...
enriched_targets as (
    select
        *,
        ...
        case
            when target_type = 'pace.zone' then {{format_pace_from_speed('target_value_one')}}
            else null
        end as target_pace_lower_range,
        case
            when target_type = 'pace.zone' then {{format_pace_from_speed('target_value_two')}}
            else null
...
indexed as (
    select
        *,
        ...
        case
            when step_dto_type = 'ExecutableStepDTO' and is_nested_step = false then step_order - 1
            when step_dto_type = 'ExecutableStepDTO' and is_nested_step = true then step_order - 2
            else null
