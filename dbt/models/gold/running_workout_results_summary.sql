...
with workout_split_summary as (
    select
        *,
        case
            when step_type = 'warmup'
                then
                    concat(
                        'Warmup: ',
                        cast(round(distance_m / 1000.0, 2) as varchar),
                        ' km for ',
                        split_pace
                    )

            when step_type = 'cooldown'
                then
                    concat(
                        'Cooldown: ',
                        cast(round(distance_m / 1000.0, 2) as varchar),
                ...
                then
                    concat(
                        ...
                        case
                            when
                                target_goal = 'time'
                                then concat(target_time_min, ' min')
                            when
                                target_goal = 'distance'
                                then
                                    concat(
                                        cast(target_distance_km as varchar),
                                        ' km'
                                    )
                            else ' NULL '
                        end,
                        case
                            when
                                target_pace_lower_range is not null
